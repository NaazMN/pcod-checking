...
def message_inbox(request):
    ...
    # Get unique sender IDs who sent messages to the current user
    message_senders = Message.objects.filter(
        receiver_id=user_id
    ).values_list('sender_id', flat=True).distinct()

    # Use a set to avoid duplicate sender entries
    processed_senders = set()
    conversations = []

    for sender_id in message_senders:
        if sender_id in processed_senders:
            continue  # Skip already processed senders
        ...
        try:
            ...
            processed_senders.add(sender_id)

        except Usertable.DoesNotExist:
            continue  # Ignore missing users
